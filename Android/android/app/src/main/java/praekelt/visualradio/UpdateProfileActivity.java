...
import praekelt.visualradio.Rest.Models.ReceivedError;
...
public class UpdateProfileActivity extends Activity {
    ...
    private void sendData() {
        ...
        if (cancel) {
            ...
            Log.d("Authentication: ", this.profileGlobal.getAuthentication());
            api.updateProfile(this.profileGlobal.getId(), "Basic " + this.profileGlobal.getAuthentication(), profile, new Callback<ReceivedProfileData>() {
                @Override
                public void success(ReceivedProfileData profile, Response response) {
                    Log.d("String: ", profile.toString());
                    //REURN TO PARENT ACTIVITY
                    profile.setAuthentication(Base64.encodeToString((username + ":" + password).getBytes(), 0));
                    returnToMain(profile);
                }

                @Override
                public void failure(RetrofitError error) {
                    ReceivedError receivedError = (ReceivedError) error.getBodyAs(ReceivedError.class);
                    switch (error.getResponse().getStatus()) {
                        case 400:
                            String alertMessage = "";
                            if(receivedError.getUsername() != null) {
                                alertMessage += "Username: " + receivedError.getUsername() + "\n";
                            }
                            if(receivedError.getFirstName() != null) {
                                alertMessage += "Name: " + receivedError.getFirstName() + "\n";
                            }
                            if(receivedError.getLastName() != null) {
                                alertMessage += "Surname: " + receivedError.getLastName() + "\n";
                            }
                            if(receivedError.getEmail() != null) {
                                alertMessage += "E-Mail: " + receivedError.getEmail() + "\n";
                            }
                            if(receivedError.getMobileNumber() != null) {
                                alertMessage += "Mobile Number:: " + receivedError.getMobileNumber() + "\n";
                            }
